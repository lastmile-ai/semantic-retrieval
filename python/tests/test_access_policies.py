...
from semantic_retrieval.document_parsers.multi_document_parser import MultiDocumentParser, ParserConfig
...
class AlwaysDenyPolicy(ResourceAccessPolicy):
    ...
    async def testDocumentReadPermission(self, document: Document, requestor: Optional[AccessIdentity] = None):
        return False
...
def test_access_passport():
    ...
    retrieved_identity = access_passport.get_identity("test-resource")
    assert retrieved_identity is not None
    assert retrieved_identity.resource == "test-resource"
...
async def test_access_policies():
    ...
    assert await always_deny_policy.testDocumentReadPermission(ingested_documents[0], AccessIdentity("abc")) == False
    assert await always_deny_policy.testPolicyPermission(AccessIdentity("abc")) == False

    assert await always_accept_policy.testDocumentReadPermission(ingested_documents[0], AccessIdentity("abc")) == True
    assert await always_accept_policy.testPolicyPermission(AccessIdentity("abc")) == True
