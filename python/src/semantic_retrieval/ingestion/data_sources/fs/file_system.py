...
class FileSystem(DataSource, Traceable):
    ...
    async def load_file(
        self, path: str, collection_id: str, run_id: str
    ...
    async def load_documents(
        self, run_id: str, filters: Optional[Any] = None, limit: Optional[int] = None
    ) -> List[RawDocument]:
        ...
        if isdir:
            ...
            for file in files:
                ...
                if os.path.isdir(subdir_path):
                    ...
                    raw_documents.extend(await subDir.load_documents(run_id))
                elif os.path.isfile(subdir_path):
                    raw_documents.append(
                        await self.load_file(subdir_path, collection_id, run_id)
                    )
        elif isfile:
            collection_id = (
                self.collection_id if self.collection_id else str(uuid.uuid4())
            )
            raw_documents.append(await self.load_file(self.path, collection_id, run_id))
        else:
