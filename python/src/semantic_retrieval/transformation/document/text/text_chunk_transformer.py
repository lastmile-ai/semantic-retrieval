...
class TextChunkTransformerParams(TextChunkConfig):
    metadataDB: Optional[DocumentMetadataDB]
...
class TextChunkTransformer(BaseDocumentTransformer, TextChunkConfig):
    def __init__(self, params: Optional[TextChunkTransformerParams] = None):
        super().__init__(params["metadataDB"], params.get("callback_manager", None))
        self.chunkSizeLimit = (
            params.chunkSizeLimit if params and hasattr(params, "chunkSizeLimit") else 500
        )
        self.chunkOverlap = (
            params.chunkOverlap if params and hasattr(params, "chunkOverlap") else 100
        )
        self.sizeFn = (
            params.sizeFn if params and hasattr(params, "sizeFn") else lambda text: len(text)
        )

        if self.chunkOverlap >= self.chunkSizeLimit:
            raise ValueError("chunkOverlap must be less than chunkSizeLimit")
