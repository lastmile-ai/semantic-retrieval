...
describe("OpenAI Embeddings transformation, embedding single fragment", () => {
  ...
  test("throws an error if the fragment exceeds max token size", async () => {
    ...
    await expect(transformer.embedFragment(fragment)).rejects.toThrowError(
      `Fragment ${fragment.fragmentId} encoded length 8192 exceeds max input tokens (8191) for model text-embedding-ada-002`
    );
  });
  ...
  test("embeds a fragment with correct data", async () => {
    ...
    const fragment = getTestDocumentFragment({
      content: "This is some example text to embed",
    });
    const embedding = await transformer.embedFragment(fragment);
    expect(embedding.fragmentId).toBe(fragment.fragmentId);
...
describe("OpenAI Embeddings transformation, embedding multiple fragments for multiple Documents", () => {
  ...
  test("embeds all fragments with correct data in one batch if they fit", async () => {
    ...
    for (const [idx, embedding] of embeddings.entries()) {
      expect(embedding.fragmentId).toBe(testFragments[idx].fragmentId);
      expect(embedding.metadata.model).toBe("text-embedding-ada-002");
