...
describe("inMemoryDocumentMetadataDB", () => {
  ...
  test("persist to, and load from CSV", async () => {
    const metadataDB = new InMemoryDocumentMetadataDB({
      [TEST_DOCUMENT_ID]: TEST_METADATA,
    });

    await metadataDB.persist(
      "__tests__/__mocks__/document/testDocumentMetadataDB.csv"
    );

    const metadataDBFromCSV = await InMemoryDocumentMetadataDB.fromCSV(
      "__tests__/__mocks__/document/testDocumentMetadataDB.csv"
    );

    const retrievedMetadata =
      await metadataDBFromCSV.getMetadata(TEST_DOCUMENT_ID);
