...
export class PineconeVectorDB extends VectorDB {
  ...
  private sanitizeMetadata(
    ...
    // Add string arrays to metadata first to prevent flattening in the next step
    for (const [key, value] of Object.entries(unsanitizedMetadata)) {
      if (Array.isArray(value) && value.every((v) => typeof v === "string")) {
        stringArrayMetadata[key] = value;
      }
    }
    ...
    const metadata: {
      ...
      ...(flatten(unsanitizedMetadata) as Record<
        string,
