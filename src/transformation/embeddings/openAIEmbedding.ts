...
export class OpenAIEmbeddings extends DocumentFragmentEmbeddingsTransformer {
  ...
  async transformDocuments(
    ...
    while (documentIdx < documents.length) {
      ...
      const currentDocumentFragmentData = await Promise.all(
      ...
      while (currentDocumentFragmentIdx < currentDocumentFragmentData.length) {
        const currentFragmentData =
          currentDocumentFragmentData[currentDocumentFragmentIdx];
        const currentDocumentFragmentEncoding = encoding.encode(
          currentFragmentData.text
        );

        // TODO: Handle this for other models when they are supported
        if (currentDocumentFragmentEncoding.length > 8191) {
          throw new Error(
        ...
        if (
          currentTextBatchSize + currentDocumentFragmentEncoding.length >
          8191
        ) {
  ...
  private async createEmbeddings(
    ...
    const { data, ...metadata } = embeddingRes;
    return data.map((embedding, idx) => ({
      ...
      metadata: {
        ...
        usage: metadata.usage as unknown as JSONObject,
      },
      attributes: {},
    }));
  }
