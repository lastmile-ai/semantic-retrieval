...
export class OpenAIEmbeddings extends DocumentFragmentEmbeddingsTransformer {
  ...
  async embedFragment(
    ...
    const fragmentEncoding = encoding.encode(fragmentData.text);

    // TODO: Handle this for other models when they are supported
    if (fragmentEncoding.length > 8191) {
      encoding.free();
      throw new Error(
        `Fragment ${fragment.fragmentId} encoded length ${fragmentEncoding.length} exceeds max input tokens (8191) for model ${this.model}`
      );
    }
    ...
    while (documentIdx < documents.length) {
      ...
      while (currentDocumentFragmentIdx < currentDocumentFragments.length) {
        ...
        if (currentFragmentEncoding.length > 8191) {
          encoding.free();
          throw new Error(
            `Fragment ${currentFragmentData.fragmentId} encoded length ${currentFragmentEncoding.length} exceeds max input tokens (8191) for model ${this.model}`
          );
        }
