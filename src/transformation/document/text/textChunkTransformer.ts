  ...
  implements TextChunkConfig
{
  ...
  protected async mergeSubChunks(
    ...
    // If keepSeparator is true, subchunks will already include separators, so we won't need
    // to add them when joining chunks
    const chunkSeparator = this.keepSeparator ? "" : separator;
    const chunkSeparatorSize = await this.sizeFn(chunkSeparator);
    ...
    for (const sc of subChunks) {
      ...
      if (
        currentChunkSize + scSize + chunkSeparatorSize >
        this.chunkSizeLimit
      ) {
        ...
        const chunk = this.joinSubChunks(currentSubChunks, chunkSeparator);
        if (chunk != null) {
          chunks.push(chunk);
        }
