  ...
  implements TextChunkConfig
{
  ...
  constructor(params?: TextChunkTransformerParams) {
    ...
    this.chunkSizeLimit = config?.chunkSizeLimit ?? this.chunkSizeLimit;
    this.chunkOverlap = config?.chunkOverlap ?? this.chunkOverlap;
    this.keepSeparator = config?.keepSeparator ?? this.keepSeparator;
    this.sizeFn = config?.sizeFn ?? this.sizeFn;
  ...
  protected subChunkOnSeparator(text: string, separator: string): string[] {
    let subChunks = text.split(separator);

    if (this.keepSeparator && separator !== "") {
      subChunks = subChunks.map((sc) => sc + separator);
    }
  ...
  protected joinSubChunks(subChunks: string[]): string | null {
    const chunk = subChunks.join().trim();
    return chunk === "" ? null : chunk;
  }

  protected async mergeSubChunks(subChunks: string[]): Promise<string[]> {
    const chunks: string[] = [];
    ...
    for (const sc of subChunks) {
      ...
      const numPrevSubChunks = prevSubChunks.length;
      if (currentSubChunks.length === 0 && numPrevSubChunks > 0) {
        let prevSubChunkOverlapStartIdx = numPrevSubChunks;
        let prevSubChunksSize = 0;
        ...
        while (prevSubChunkOverlapStartIdx > 0) {
          const nextPrevSubChunkSize = await this.sizeFn(
            prevSubChunks[prevSubChunkOverlapStartIdx - 1]
          );

          if (
            nextPrevSubChunkSize + prevSubChunksSize > this.chunkOverlap ||
            nextPrevSubChunkSize + prevSubChunksSize + scSize >
              this.chunkSizeLimit
          ) {
            break;
          }

          prevSubChunksSize += nextPrevSubChunkSize;
          prevSubChunkOverlapStartIdx--;
        }

        while (prevSubChunkOverlapStartIdx < numPrevSubChunks) {
          currentSubChunks.push(prevSubChunks[prevSubChunkOverlapStartIdx]);
          prevSubChunkOverlapStartIdx++;
        }

        currentChunkSize = prevSubChunksSize;
      }

      if (currentChunkSize + scSize > this.chunkSizeLimit) {
        // Current subChunk would cause the chunk to exceed the size limit, so create the current chunk
        // before starting a new one with the current subChunk
        const chunk = this.joinSubChunks(currentSubChunks);
        if (chunk != null) {
          chunks.push(chunk);
        }
